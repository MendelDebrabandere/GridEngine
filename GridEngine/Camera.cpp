...
namespace grid
{
	...
	void Camera::Initialize(float _aspectRatio, float _fovAngle, Vector3 _origin)
	{
		...
		fovAngle = _fovAngle;
		fov = tanf((fovAngle * static_cast<float>(TO_RADIANS)) / 2.f);
	...
	void Camera::DoKeyboardInput(float deltaTime, float moveSpeed)
	{
		//Keyboard Input
	...
	void Camera::DoMouseInput(float deltaTime, float moveSpeed, float mouseSens)
	{
		//Mouse Input
		int mouseX{}, mouseY{};
